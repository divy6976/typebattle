...
export default function NavBar() {
  ...
  return (
    <nav className="relative w-full overflow-hidden rounded-3xl border border-white/12 bg-[#070d19]/75 px-4 py-0.5 backdrop-blur-2xl shadow-[0_18px_50px_-32px_rgba(0,0,0,0.95)]">
      ...
      <div className="relative flex h-[48px] items-center justify-between">
        <Link href="/" className="font-display flex items-center gap-1.5 text-[29px] font-extrabold leading-none tracking-tight transition-transform duration-300 hover:scale-[1.01]">
        ...
        <div className="flex h-full items-center gap-1 font-mono">
          {NAV_ITEMS.map((item) => {
            const active = isActive(pathname, item.href);
            const itemClassName = [
              "relative flex h-full items-center gap-2 rounded-xl px-3 text-[16px] font-semibold leading-none transition-all duration-200",
              active
          ...
          {soonMessage ? (
            <span className="ml-2 text-[12px] font-semibold text-[#facc15]">{soonMessage}</span>
          ) : null}
        </div>
